...
public class StaffController : ControllerBase
{
    ...
    [HttpGet("Search")]
    public async Task<IActionResult> Search([FromQuery] string? staffId, [FromQuery] int? gender, [FromQuery] int? startYear, [FromQuery] int? endYear)
    {
        var result = await _staffRepository.Search(staffId, gender, startYear, endYear);
        return Ok(result);
    }
}
